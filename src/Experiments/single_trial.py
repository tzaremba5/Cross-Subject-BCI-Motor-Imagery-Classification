...
def single_trial(X, Y, num_classes, model_config, train_config,
    ...
    model.save_weights(
        weights_initial_path
    )  # Saves the empty weights rigth after the models created
    ...
    for iteration in range(3):
        ...
        for train_index, test_index in sss.split(
                X, Y):  # gets the train and test indecies
            model.load_weights(weights_initial_path
                               )  # Restores the model to the inital weights
