...
#
# 
def Cross_Subject(X_main, Y_main, X_other, Y_other, num_classes, class_weights, weights_initial, model_config, train_config):
	...
	model.fit(X_other, Y_other, validation_data = (X_main, Y_main), epochs = train_config['epochs'],
	...
	for iteration in range(3):
		...
		for train_index, test_index in sss.split(X_main, Y_main): # gets the train and test indecies
			model.load_weights(weights_initial) # Restores the model to the inital weights
...
if __name__ == '__main__':
	...
	X_main, Y_main, X_other, Y_other, num_classes, class_weights = create_dataset_CSCD(competition, subject)
